...
The API proxy's first call might use a ServiceCallout policy to securely fetch the secret from Secret Manager over a private channel.

The proxy then populates a KVM with this value (using JavaScript or a dedicated policy) for subsequent, low-latency access.